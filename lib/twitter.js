...
const sendTwitter = (ctx, twitter) => {
	...
	if (styles.keyboard) {
		...
		if (styles.keyboard == 'count') {
			...
			commentStatus += ` ${twitter.comment_cout}` //TODO CHECK
	...
	ctx.bot.telegram.sendMessage(ctx.chat_id, output, {
		parse_mode: 'HTML',
		disable_web_page_preview: styles.previewLink || false,
